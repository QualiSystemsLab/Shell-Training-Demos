...
from cli_commands import get_all_ips
from cloudshell.api.cloudshell_api import CloudShellAPISession
from cloudshell.shell.core.session.logging_session import LoggingSessionContext
from cloudshell.logging.qs_logger import get_qs_logger

class ComputeDemoDriver (ResourceDriverInterface):
    ...
    def get_inventory(self, context):
        ...
        '''
        resource = ComputeDemo.create_from_context(context)
    ...
    def _get_cli_from_context(self, context, api, logger=None):
        """
        :param ResourceCommandContext context:
        :param CloudShellAPISession api:
        :param logging.logger logger:
        :return: cloudshell-cli session
        :rtype: CreateSession
        """
        resource = ComputeDemo.create_from_context(context)
        ...
        password = api.DecryptPassword(password).Value
        cli = CreateSession(host=host, username=user_name, password=password, logger=logger)
        return cli
    ...
    def send_custom_command(self, context, command):
        ...
        msg = "Sending command {}...".format(command)
        api.WriteMessageToReservationOutput(res_id, msg)
        ...
        with LoggingSessionContext(context) as logger:
            cli = self._get_cli_from_context(context, api, logger)
            if commands_list:
                outp = cli.send_commands_list(commands_list)
            else:
                outp = cli.send_terminal_command(command)
        return outp

    def get_all_ips(self, context):
        """
        :param ResourceCommandContext context:
        :return:
        """
        api = self._get_api_from_context(context)
        res_id = context.reservation.reservation_id
        msg = "Sending CLI Call to device for all IPs..."
        api.WriteMessageToReservationOutput(reservationId=res_id, message=msg)
        with LoggingSessionContext(context) as logger:
            cli = self._get_cli_from_context(context, api, logger)
